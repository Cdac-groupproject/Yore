...
public class SecurityConfig {
    ...
    // ✅ CORS configuration for allowing frontend (localhost:5173)
    @Bean
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            .cors(cors -> cors.configurationSource(corsConfigurationSource())) // ✅ enable CORS from above config
            .csrf().disable()
            .authorizeHttpRequests(requests -> requests
                .requestMatchers("/signin", "/sign-up","/verify-user", "/edit-profile").permitAll()
                .requestMatchers("/bidder/**").hasRole("BIDDER")
                .requestMatchers("/manager/**").hasRole("MANAGER")
                .requestMatchers("/auctioneer/**").hasRole("AUCTIONEER")
                .anyRequest().authenticated()
